...
Partial Class rebatebill
    ...
    Private Sub BindReport()
        Try
            ...
            rpt.Load(Server.MapPath("~/reports/rebatebilSearchingsheet.rpt"))
            ...
            rpt.RecordSelectionFormula = "{invoice.consumer_no} = '" & txtconsumerno.Text.ToString & "' and {invoice.status} = 6 and {invoice.rebate_status} = 1"
	...
	Protected Sub btnins_Click(ByVal sender As Object, ByVal e As System.EventArgs)
		...
		Try
		con.Open()
            cmd1 = New SqlCommand("insert into duplicatebill_history  (Consumer_No,Category,zone, bill_type,createdby, createdon) SELECT  consumer_no, category,zone,'Rebate Bill 25% (Searching Sheet)', '" & se & "', GETDATE()FROM invoice WHERE  consumer_no = '" & txtconsumerno.Text & "'", con)
