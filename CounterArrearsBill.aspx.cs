...
public partial class CounterArrearsBill : System.Web.UI.Page
{
    ...
    private void BindReport()
    {
        try
        {
            ...
            rpt.Load(Server.MapPath("~/reports/arrears_only.rpt"));
    ...
    protected void btnins_Click(object sender, System.EventArgs e)
    {
        ...
        {
            con.Open();
            cmd1 = new SqlCommand("insert into duplicatebill_history  (Consumer_No,Category,zone, bill_type,createdby, createdon,issuedate,duedate,cnic,phone) SELECT  consumer_no, category,zone,'Arrears Bill', '" + se + "', GETDATE(),GETDATE(),dateadd(day, 10, (CONVERT(VARCHAR(10), getdate(), 111))),'" + txtCNIC.Value + "','" + txtphone.Value + "' FROM invoice WHERE  consumer_no = '" + txtconsumerno.Text + "'", con);
