...
public partial class CounterBill : System.Web.UI.Page
{
    ...
    private void BindReport()
    {
        try
        {
            ...
            rpt.RecordSelectionFormula = "{invoice.consumer_no} = '" + txtconsumerno.Text + "' and {invoice.status} = 1 and {invoice.rebate_status} = 1";
    ...
    protected void btnins_Click(object sender, System.EventArgs e)
    {
        ...
        {
            con.Open();
            cmd1 = new SqlCommand("insert into duplicatebill_history  (Consumer_No,Category,zone, bill_type,createdby, createdon,issuedate,duedate,cnic,phone) SELECT  consumer_no, category,zone,'Rebate Bill 25%', '" + se + "', GETDATE(),GETDATE(),dateadd(day, 10, (CONVERT(VARCHAR(10), getdate(), 111))),'"+txtCNIC.Value+"','"+txtphone.Value+"' FROM invoice WHERE  consumer_no = '" + txtconsumerno.Text + "'", con);
