...
public partial class CounterCurrentOnly : System.Web.UI.Page
{
    ...
    protected void btnins_Click(object sender, System.EventArgs e)
    {
        ...
        {
            con.Open();
            cmd1 = new SqlCommand("insert into duplicatebill_history  (Consumer_No,Category,zone, bill_type,createdby, createdon,issuedate,duedate,cnic,phone) SELECT  consumer_no, category,zone,'Current Only Bill', '" + se + "', GETDATE(),GETDATE(),dateadd(day, 10, (CONVERT(VARCHAR(10), getdate(), 111))),'" + txtCNIC.Value + "','" + txtphone.Value + "' FROM invoice WHERE  consumer_no = '" + txtconsumerno.Text + "'", con);
